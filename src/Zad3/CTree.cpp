...
CTree::CTree(std::string &str_equation) {
    vFirstInit();
    std::string str_equation_onp = str_equation;
    vMakeTree(str_equation_onp);
}
...
double CTree::dCalculate() {
    ...
    if(pc_root== nullptr){
        ...
        d_result = dCalculateHelper(pc_root);
    }
    return d_result;
}
...
void CTree::vMakeTree(std::string &str_onp) { //wszystko rodzielone spacja
    delete pc_root;
    std::stringstream  ss_stream;
    for(int i=0; i<str_onp.size(); i++){
        if(str_onp[i]==' '){
...
double CTree::dCalculateHelper(CNode *pc_node) {
    double d_result;
    if(pc_node== nullptr){
        d_result=0;
    }else {
        ...
        if (pc_node->eGetType() == Operators::NUMBER) {
            ...
            if (pc_node->pcGetLeft() != nullptr) {
                double d_arg = dCalculateHelper(pc_node->pcGetLeft());
            ...
            if (pc_node->pcGetLeft() != nullptr && pc_node->pcGetRight() != nullptr) {
                double d_arg1 = dCalculateHelper(pc_node->pcGetLeft());
                double d_arg2 = dCalculateHelper(pc_node->pcGetRight());
...
bool CTree::bAddNode(CNode *pc_node) {
    ...
    */
    if(pc_root == nullptr){
        this->pc_root = pc_node;
        return true;
    }else return bAddHelper(pc_node,pc_root);
}
bool CTree::bAddHelper(CNode *pc_node, CNode *pc_destination){
    bool b_result;
    if(pc_node == nullptr){
        b_result= false;
    }else if(pc_destination== nullptr){
        b_result= false;
    }else{
        b_result= true;
        CNode* temp =pc_destination;
        bool b_loop=true;
        while (b_loop){
            if(temp->i_counter_left>0){
                temp->i_counter_left+= pc_node->iGetFillSize()-1;
...
std::string CTree::strHelper(CNode* pc_node) {
    ...
    std::stringstream ss_stream;
    ss_stream<<"(";
    if(e_operator==Operators::MINUS || e_operator==Operators::PLUS || e_operator==Operators::MULL || e_operator==Operators::DIVIDE){
        if(pc_node->pcGetLeft()!= nullptr && pc_node->pcGetRight()!= nullptr){
...
CTree CTree::operator+(const CTree &other) {
    if(pc_root == nullptr || other.pc_root == nullptr){
        CTree c_result_tree;
        if(pc_root == other.pc_root){
            c_result_tree =  CTree();
        }else if(this->pc_root == nullptr){
            c_result_tree = CTree(other);
        }else{
            c_result_tree = CTree(*this);
        }
